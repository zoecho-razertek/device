...
import ic2fa          from './assets/ic_bpart_2fa.png'
...
import icSosSearch             from './assets/ic_bpart_sos_search.png'
import icSosSet                from './assets/ic_bpart_sos_set.png'
...
import icBpartDevice  from './assets/ic_bpart_device.png'
import icAddonCateye  from './assets/ic_addon_cateye.png'
...
import icKeyFinger    from './assets/ic_key_finger.png'
import icKeyFace      from './assets/ic_key_face.png'
import icKeyHand      from './assets/ic_key_hand.png'
...
const SS_ITEMS = [
  ...
  { id: '2fa',     title: '開鎖模式',     sub: '一般',                  subColor: '#666', icon: ic2fa      },
  { id: 'key',     title: '感應開鎖管理', sub: '卡片/Ｍ-Key\n生物辨識', subColor: '#666', icon: icKey      },
...
const SS2_ITEMS = [
  ...
  { id: 'addon', title: '周邊設備管理', sub: '安裝與移除',   subColor: '#666', icon: icBpartDevice },
  { id: 'cateye', title: '電子貓眼', sub: '已安裝的週邊設備', subColor: '#666', icon: icAddonCateye },
...
const KEY_ITEMS = [
  ...
  { id: 'finger',title: '指紋管理',     sub: '已啟用', subColor: '#337c05', icon: icKeyFinger },
  { id: 'face',  title: '臉部資料管理', sub: '未安裝', subColor: '#999',    icon: icKeyFace   },
  { id: 'hand',  title: '掌靜脈管理',   sub: '已啟用', subColor: '#337c05', icon: icKeyHand   },
]
...
const PASSCODE_ITEMS = [
  ...
  { id: 'query_sos',    title: '查詢求救密碼', sub: '緊急時用來求救\n的密碼',      subColor: '#666', icon: icSosSearch             },
  { id: 'set_sos',      title: '設定求救密碼', sub: '緊急時用來求救\n的密碼',      subColor: '#666', icon: icSosSet                },
]
      ...
      <div className="ss-grid">
        {SS2_ITEMS.map((item) => (
          <div key={item.title} className="ss-cell"
            style={['lock_info', 'probe', 'controller', 'addon', 'cateye', 'reset'].includes(item.id) ? { cursor: 'pointer' } : undefined}
            onClick={
              item.id === 'lock_info'    ? () => setPage('lock_info') :
              item.id === 'probe'        ? () => setPage('probe_addon') :
              item.id === 'controller'   ? () => setPage('controller_addon') :
              item.id === 'addon'        ? () => setPage('addon_manage') :
              item.id === 'cateye'       ? () => setPage('cateye') :
  ...
  return (
    <div className="ss-page">
      ...
      <div className="ss-grid">
        {SS_ITEMS.map((item) => (
          <div key={item.title} className="ss-cell"
            style={['passcode','key','volume','2fa','wifi','qrcode'].includes(item.id) ? { cursor: 'pointer' } : undefined}
            onClick={
              item.id === 'passcode' ? () => setPage('passcode_setting') :
              item.id === 'key'      ? () => setPage('key_management') :
              item.id === 'volume'   ? () => setPage('function_setting') :
              item.id === '2fa'      ? () => setPage('unlock_mode') :
          ...
          >
            <div className="ss-cell-text">
              ...
              {(item.id === '2fa'
                ? (unlockMode ? '雙重驗證' : '一般')
                : item.id === 'wifi' && wifiConnected ? '已連線'
                : item.sub
              ).split('\n').map((line, i) => (
                <span key={i} className="ss-cell-sub" style={{ color: item.id === '2fa' && unlockMode ? '#DD9F18' : item.id === 'wifi' && wifiConnected ? '#337C05' : item.subColor }}>{line}</span>
              ))}
            </div>
            <img src={item.icon} alt="" className="ss-cell-icon" />
          </div>
